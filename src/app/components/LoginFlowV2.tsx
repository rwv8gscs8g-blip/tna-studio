...
export default function LoginFlowV2() {
  ...
  async function onSubmitPassword(e: React.FormEvent) {
    ...
    try {
      ...
      if (result?.error) {
        console.error("[LoginFlowV2] Erro no login:", result.error);
      ...
      if (result?.ok) {
        console.log("[LoginFlowV2] Login bem-sucedido, redirecionando...");
        setIsSubmitting(false);
        ...
        try {
          ...
          // Regras definitivas de redirecionamento
          if (callbackUrl === "/" || !callbackUrl) {
            if (userRole === "ARQUITETO") {
              redirectUrl = "/arquiteto/relatorios";
            } else if (userRole === "ADMIN") {
              redirectUrl = "/admin/relatorios";
            } else if (userRole === "MODELO") {
              redirectUrl = "/modelo/home";
            } else if (userRole === "CLIENTE") {
              redirectUrl = "/modelo/home";
            } else if (userRole === "SUPERADMIN") {
              redirectUrl = "/super-admin/certificates";
            } else {
              redirectUrl = "/";
            }
          }
          ...
          console.warn("[LoginFlowV2] Erro ao buscar sessão após login:", fetchErr);
      ...
      console.warn("[LoginFlowV2] Resposta inesperada do signIn:", result);
      setErrorMessage("Erro inesperado. Tente novamente.");
      setIsSubmitting(false);
    } catch (err: any) {
      console.error("[LoginFlowV2] Erro ao fazer login:", err);
  ...
  if (!showLoginForm) {
    // Tela inicial com dois botões principais
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "2rem" }}>
        ...
        >
          Cadastre-se (Modelo ou Cliente)
        </Link>

        <p style={{ fontSize: 13, color: "#9ca3af", marginTop: "0.5rem", textAlign: "center" }}>
          Token mágico por WhatsApp/SMS (em breve)
        </p>
      </div>
    );
  }
