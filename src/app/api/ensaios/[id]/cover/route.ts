/**
 * GET /api/ensaios/[id]/cover
 * 
 * Retorna URL assinada efêmera para a foto de capa do ensaio.
 * 
 * SEGURANÇA:
...
export async function GET(
  ...
  try {
    ...
    // Gerar URL assinada para cover
    const coverImageKey = (ensaio as any).coverImageKey || (ensaio as any).coverImageUrl;
    
    if (!coverImageKey) {
      return NextResponse.json(
        { error: "Foto de capa não encontrada." },
        { status: 404 }
      );
    }

    try {
      const signedUrl = await getSignedUrlForKey(coverImageKey, { expiresInSeconds: 120 });
    ...
    console.error("[API] Erro ao buscar cover do ensaio:", error);
    return NextResponse.json(
