...
- Dados existentes preservados (usuário ARQUITETO mantido)
...
**Confirmação:** `npm run debug:db` mostra todos os 5 usuários com roles corretos.
...
- ✅ Adicionada rota pública `/modelo/signup`

**Rotas públicas:**
- `/signin` - Página de login
- `/modelo/signup` - Auto-cadastro de modelos (NOVO)
...
**Página de signup:**
**Arquivo:** `src/app/modelo/signup/page.tsx` (NOVO)
...
**Endpoint de API:**
**Arquivo:** `src/app/api/auth/signup/modelo/route.ts` (NOVO)
...
**Proteção:**
- ✅ Não há endpoint para MODELO atualizar seu próprio perfil
- ✅ Somente ARQUITETO poderá editar dados de qualquer usuário (implementação futura)
...
4. ✅ `scripts/apply-role-enum-migration.ts` - Script para aplicar migration manualmente (NOVO)
...
### Outras correções
11. ✅ `src/app/admin/users/page.tsx` - Permite acesso de ARQUITETO e ADMIN
12. ✅ `src/app/api/admin/users/route.ts` - Default role ajustado para `Role.MODELO`
...
6. Deve conseguir criar ensaios
...
2. Credenciais:
   ...
   - Senha: `Admin@2025!`
3. Deve fazer login com sucesso
4. Acesse: `http://localhost:3000/admin/users`
5. Deve ver a lista de usuários (ADMIN pode acessar)
6. Tente acessar: `http://localhost:3000/arquiteto/ensaios`
7. Deve redirecionar para `/signin` (apenas ARQUITETO pode acessar)
...
7. Verifique com `npm run debug:db` que o usuário foi criado com role `MODELO`
